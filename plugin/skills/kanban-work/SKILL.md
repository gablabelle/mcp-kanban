...
allowed-tools: Bash, Read, Write, Edit, Grep, Glob, mcp__kanban__open_board, mcp__kanban__list_columns, mcp__kanban__list_tickets, mcp__kanban__get_ticket, mcp__kanban__move_ticket, mcp__kanban__update_ticket, mcp__kanban__complete_subtask, mcp__kanban__create_subtask
---
...
## Step 0: Start the board (MANDATORY FIRST STEP)
...
1. **Before starting**: Move the subtask to "in progress" with `mcp__kanban__move_ticket`
2. **Do the work**: Implement the changes — read files, write code, run tests, etc.
3. **Update the ticket**: Use `mcp__kanban__update_ticket` to add implementation notes to the subtask description — what was done, what files were changed, any decisions made
4. **Mark complete**: Use `mcp__kanban__complete_subtask` to move the subtask to Done

## Step 5: Complete the story

Once all subtasks are done:
1. Use `mcp__kanban__update_ticket` to update the parent story's description with a summary of what was implemented
2. Move the story to "review" or "done" column with `mcp__kanban__move_ticket`
...
- If you discover additional work needed while implementing, create new subtasks with `mcp__kanban__create_subtask` rather than expanding the scope of existing ones
